...
/**
 * Strips markdown down to plain text for the note-card preview — syntax
 * markers gone, images dropped, blocks joined with spaces.
 */
export function markdownPreview(text: string, maxLength = 220): string {
  const parts: string[] = [];
  for (const block of parseBlocks(text)) {
    switch (block.type) {
      ...
      case "bullets":
      case "numbered":
        parts.push(block.items.map((it) => inlineText(it.children)).join(" · "));
        break;
      case "code":
        parts.push(block.text.replaceAll("\n", " "));
        break;
      case "hr":
        break;
    }
